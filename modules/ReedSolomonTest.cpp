/**
 * @file ReedSolomonTest.cpp
 * @brief Unit tests for the Reed-Solomon error correction library.
 * @author FluffyJay1
 * @link https://github.com/FluffyJay1/ReedSolomon
 *
 * This file contains unit tests for the Reed-Solomon error correction library
 * available at https://github.com/FluffyJay1/ReedSolomon.
 */
#include <time.h>
#include <iomanip>
#include <iostream>
#include <fstream>
#include "myRSmodule.cpp"
...
void test()
{
    ...
    vector<unsigned int> data1 = { 0x40, 0xd2, 0x75, 0x47, 0x76, 0x17, 0x32, 0x06, 0x27, 0x26, 0x96, 0xc6, 0xc6, 0x96, 0x70, 0xec };
    Poly<unsigned int> msg(k, data1);
    Poly<unsigned int> a(k + nsym, data1);
    ReedSolomon<unsigned int> rs(bits);
...
{
    vector<unsigned int> originalData(k + nsym);
    for (int i = 0; i < k; i++)
        originalData[i] = rand() % (1 << bits);
    Poly<unsigned int> msg(k, originalData);
    Poly<unsigned int> a(k + nsym, originalData);
    ReedSolomon<unsigned int> rs(bits);
